...
//{	
mysql_query("UPDATE  `users` SET  `FirstName` =  '$FirstName',`MiddleName` =  '$MiddleName',`LastName` =  '$LastName',`DateOfBirth` =  '$DateOfBirth', `e-mail` ='$e_mail', `avatar` =  '$avatar' WHERE  `id` ='$id'",$db) or die(mysql_error());
//}
$result = mysql_query("SELECT * FROM users WHERE login='$login'",$db); //извлекаем из базы все данные о пользователе с введенным логином
	...
	$_SESSION['DateOfBirth']=$myrow['DateOfBirth'];
	$_SESSION['e-mail']=$myrow['e-mail'];
	$_SESSION['picture']=$myrow['avatar'];
